"""CLI entry point for depwarden using Typer."""
...
from depwarden import __version__
from depwarden.bloat import analyze_bloat
from depwarden.config import DepwardenConfig
from depwarden.deps_reader import read_dependencies
from depwarden.health_score import calculate_health_score
from depwarden.models import ScanResult
from depwarden.reporter import report_json, report_rich
from depwarden.security import scan_vulnerabilities

app = typer.Typer(
    name="depwarden",
    help="🛡️  depwarden — Escape Dependency Hell. Scan, audit, and fix your Python dependencies.",
...
def scan(
    ...
    # --- Load config from [tool.depwarden] in pyproject.toml ---
    config = DepwardenConfig.from_pyproject(str(project_path))
    ...
    if format == "rich":
        ...
        with Live(update_spinner("Initializing depwarden engine..."), refresh_per_second=10, transient=True) as live:
            ...
            if include_phase2:
                try:
                    from depwarden.scanner import scan_imports
                    from depwarden.unused import find_unused
                    from depwarden.missing import find_missing
                    from depwarden.utils import filter_third_party, get_module_to_package_map
...
@app.command()
def version() -> None:
    """Show depwarden version."""
    console.print(f"depwarden v{__version__}")
